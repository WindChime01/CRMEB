...
class LiveStudio extends ModelBasic
{
    ...
    public static function setLiveWhere($where, $model = null, $alias = '')
    {
        ...
        $model = $model->join('__SPECIAL__ S', 'L.special_id=S.id', 'LEFT')->where(['S.is_del'=>0,'S.status'=>1]);
        if (isset($where['special_id']) && $where['special_id']) $model = $model->where("{$alias}special_id", $where['special_id']);
        if (isset($where['stream_name']) && $where['stream_name'] != '') $model = $model->where($alias . 'stream_name|' . $alias . 'live_title', "LIKE", "%$where[stream_name]%");
    ...
    {
        $data = self::setLiveWhere($where, null, 'L')->field('L.*,S.is_del,S.status,S.pay_type,S.member_pay_type')->order('L.sort desc,L.add_time desc')
