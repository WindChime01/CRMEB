...
class SpecialSource extends ModelBasic
{
    ...
    public static function setSpecialWhere($special_id)
    {
        $model = self::alias('s')->where(['s.special_id' => $special_id, 'l.is_del' => 0, 'l.is_show' => 1, 'l.status' => 1])
            ->join('Special l', 's.source_id=l.id')->field('l.title,l.image,l.type,l.is_show,l.is_del,l.light_type,l.is_light,s.special_id,s.source_id,s.sort,s.pay_status,s.id');
        return $model;
    }
     ...
     * @param int $limit
     */
    public static function getSpecialList($special_id = 0, $page = 1, $limit = 20, $order = 0)
    {
        $data = self::setSpecialWhere($special_id)->page($page, $limit);
        if ($order) {
            $data = $data->order('s.sort asc,s.id asc')->select();
        } else {
            $data = $data->order('s.sort desc,s.id desc')->select();
        }
        $count = self::setSpecialWhere($special_id)->count();
        return compact('data', 'count');
    }
