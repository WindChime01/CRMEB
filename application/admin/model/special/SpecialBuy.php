...
class SpecialBuy extends ModelBasic
{
    ...
    public static function getOrderPayList($uid, $page = 1, $limit = 20)
    {
        $list = self::where(['a.uid' => $uid, 'a.is_del' => 0])->alias('a')->group('a.special_id')->join('__SPECIAL__ s', 's.id=a.special_id')
            ->field('a.*,s.title')->order('a.add_time desc')->page((int)$page, (int)$limit)->select();
        return $list;
    }
}
