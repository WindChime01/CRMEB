...
class UserBill extends ModelBasic
{
    ...
    {
        $list = self::setOneWhere($where, $uid)->order('add_time desc')->page($where['page'], $where['limit'])
    ...
    public static function getOneBalanceChangList($where)
    {
        $list = self::setWhereList(
            $where,
            ['system_add', 'pay_product', 'extract', 'extract_fail', 'pay_goods', 'pay_sign_up', 'pay_sign_up_refund', 'pay_product_refund', 'pay_data_download', 'pay_data_download_refund', 'pay_test_paper', 'pay_test_paper_refund', 'recharge', 'system_sub'],
    ...
    public static function setWhereList($where, $type = '', $field = [], $category = 'gold_num')
    {
        ...
        return ($list = $models->order('id desc')->select()) && count($list) ? $list->toArray() : [];
    }
