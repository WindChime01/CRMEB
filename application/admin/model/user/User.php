...
class User extends ModelBasic
{
     ...
     *
     */
    public static function setTeacherWhere($where)
    {
        if (isset($where['order']) && $where['order'] != '') {
            $model = self::order(self::setOrder($where['order']));
        } else {
            $model = self::order('uid desc');
        }
        if (isset($where['nickname']) && $where['nickname'] != '') {
            $model = $model->where('nickname|uid|phone', 'LIKE', "%$where[nickname]%");
        }
        if (isset($where['identitys']) && $where['identitys'] != '') {
            if ($where['identitys'] == 0) $identitys = [0];
            else if ($where['identitys'] == 1) $identitys = [0, 1];
            else $identitys = [0, 1, 2];
            $model = $model->where('identitys', 'in', $identitys);
        }
        return $model;
    }
     ...
     * @return array
     */
    public static function add_teacher_user_list($where)
    {
        $model = self::setTeacherWhere($where);
        $list = $model->page((int)$where['page'], (int)$where['limit'])->select();
        $count = self::setTeacherWhere($where)->count();
    ...
    public static function setWhere($where)
    {
        ...
        if ($where['status'] != '') {
            $model = $model->where('u.status', $where['status']);
        }
        ...
        if ($where['pay_count'] !== '') {
            if ($where['pay_count'] == '-1') $model = $model->where('u.pay_count', 0);
            else $model = $model->where('u.pay_count', '>', $where['pay_count']);
        }
        switch ($where['is_promoter']) {
            ...
            case '2':
                ...
                break;
            case '3':
                $model = $model->where('u.business', 1);
                break;
        }
        return $model;
    }
    ...
    public static function getUserList($where)
    {
        ...
        $list = $model->alias('u')->field('u.*')->page((int)$where['page'], (int)$where['limit'])->select();
    ...
    */
    ...
    {
        $list = self::setCommissionWhere($where)->page((int)$where['page'], (int)$where['limit'])->select();
        count($list) && $list = $list->toArray();
    ...
    public static function setCommissionWhere($where)
    {
        ...
        if (isset($where['price_max']) && isset($where['price_min']) && $where['price_max'] > 0 && $where['price_min'] > 0) {
    ...
    public static function getCountData($uid)
    {
        $pay_count = SpecialBuy::where('uid', $uid)->where('is_del', 0)->count();
        $data_page = DataDownloadBuy::where(['uid'=>$uid,'is_del'=>0])->count();
        $test_count = TestPaperObtain::where(['uid'=>$uid,'is_del'=>0])->count();
        return compact('pay_count','data_page','test_count');
    }
   ...
   */
    ...
    public static function agentSystemPage($where = array())
    {
        ...
        foreach ($data as &$item) {
            ...
            $item['extract_count_price'] = UserExtract::getUserCountPrice($item['uid']);//累计提现金额
            $item['extract_count_num'] = UserExtract::getUserCountNum($item['uid'], $where);//提现次数
            $item['order_price'] = count($uids) ? StoreOrder::where('uid', 'in', $uids)->where(['paid' => 1, 'refund_status' => 0, 'is_del' => 0])->where('type', 'in', '0,1,2')->sum('pay_price') : 0;//订单金额
            $item['order_count'] = count($uids) ? StoreOrder::where('uid', 'in', $uids)->where(['paid' => 1, 'refund_status' => 0, 'is_del' => 0])->where('type', 'in', '0,1,2')->count() : 0;//订单数量
            $item['spread_name'] = '暂无';
            if ($spread_uid = self::where('uid', $item['uid'])->value('spread_uid')) {
                if ($user = self::where('uid', $spread_uid)->field(['uid', 'nickname'])->find()) {
                    $item['spread_name'] = $user['nickname'] . '/' . $user['uid'];
    ...
    public static function getSpreadBadge($where)
    {
        ...
        $uids2 = [];
        foreach ($listuids as $item) {
            $Listuids1 = self::where('spread_uid', $item['uid'])->field('uid')->select();
            $newUids1 = [];
            foreach ($Listuids1 as $val) {
                $newUids1[] = $val['uid'];
            }
            $uids1 = $newUids1;
            unset($newUids1);
            if (count($uids1)) {
                ...
                $uidTwo = $newUids1;
                unset($newUids1);
                $uids1 = array_merge($uids1, $uidTwo);
                $uids1 = array_unique($uids1);
                $uids1 = array_merge($uids1);
                $uids2 = array_merge($uids2, $uids1);
            }
        }
        ...
        if ($data['sum_count']) {
            ...
            //订单总数
            $data['order_count'] = StoreOrder::where('uid', 'in', $uids2)->where(['paid' => 1, 'refund_status' => 0, 'is_del' => 0])->where('type', 'in', '0,1,2')->count();
            //订单金额
            $data['pay_price'] = StoreOrder::where('uid', 'in', $uids2)->where(['paid' => 1, 'refund_status' => 0, 'is_del' => 0])->where('type', 'in', '0,1,2')->sum('pay_price');
            //可提现金额
