...
class QuestionsCategpry extends AuthController
{
    ...
    {
        $category = QuestionsCategpryModel::taskCategoryAll(2,$this->merchantId);
        return Json::successful($category);
    }

    public function add_cate_list()
    {
        $category = QuestionsCategpryModel::where(['pid' => 0,'mer_id' => $this->merchantId, 'is_del' => 0])->select();
    ...
    public function save($id = 0)
    {
        ...
        if ($id) {
            ...
            if (!$cate['pid'] && $post['pid'] && QuestionsCategpryModel::be(['pid' => $id,'mer_id' => $this->merchantId, 'is_del' => 0])) return Json::fail('无法移动有下级的分类');
            if (QuestionsCategpryModel::where(['title' => $post['title'],'mer_id' => $this->merchantId, 'is_del' => 0])->where('id', '<>', $id)->count() >= 1) return Json::fail('分类名称已存在');
    ...
    public function delete($id = 0)
    {
        ...
        $cate = QuestionsCategpryModel::get($id);
        if (!$cate['pid']) {
            $count = QuestionsCategpryModel::where(['pid' => $id, 'mer_id' => $this->merchantId, 'is_del' => 0])->count();
