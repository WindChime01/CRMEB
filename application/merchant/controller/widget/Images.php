...
namespace app\merchant\controller\widget;

use Api\AliyunOss;
use app\merchant\model\system\SystemAttachment as SystemAttachmentModel;
use app\merchant\model\system\SystemAttachmentCategory as Category;
...
class Images extends AuthController
{
    ...
    public function get_image_cate()
    {
        ...
        return Json::successful(Category::getAll($mer_id));
    }
    ...
    public function upload()
    {
        ...
        try {
            ...
            if ($res) {
                SystemAttachmentModel::attachmentAdd($mer_id, $res['key'], $title, 0, 'image/jpg', $res['url'], $res['url'], $pid, 1, time());
    ...
    public function moveimg($imgaes)
    {
        ...
        $formbuider[] = Form::select('pid', '选择分类')->setOptions(function () {
            $mer_id = $this->merchantId;
            $list = Category::getCateList('', $mer_id);
