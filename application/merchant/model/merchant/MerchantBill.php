...
class MerchantBill extends ModelBasic
{
    ...
    public static function setWhere($where)
    {
        ...
        if ($where['category'] && $where['category'] != '') $model = $model->where('category', $where['category']);
        if ($where['category'] == 'now_money') {
            $model = $model->where('type', 'in', ['extract', 'gold_extract', 'user_refund', 'user_pay']);
        } else {
    ...
    {
        $model = self::setWhere($where)->order('add_time desc');
    ...
    public static function SaveExport($where)
    {
        ...
        if ($where['category'] == 'now_money') {
            $filename = '资金流水' . time() . '.xlsx';
        } else {
            $filename = $gold_name . '流水' . time() . '.xlsx';
        }
