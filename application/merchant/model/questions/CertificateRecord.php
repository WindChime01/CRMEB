...
class CertificateRecord extends ModelBasic
{
    ...
    public static function setWhere($where)
    {
        ...
        if (isset($where['mer_id']) && $where['cid'] != 0) $model = $model->where(['r.cid' => $where['cid']]);
        if (isset($where['mer_id']) && $where['title'] != '') $model = $model->where('r.uid|r.nickname', 'like', "%$where[title]%");
        if (isset($where['mer_id']) && $where['mer_id'] != '') $model = $model->where('c.mer_id', $where['mer_id']);
        return $model;
    }
