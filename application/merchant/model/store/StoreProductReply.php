...
class StoreProductReply extends ModelBasic
{
    ...
    public static function setWhere($where)
    {
        ...
        if (isset($where['product_id']) && $where['product_id'] > 0) $model = $model->where('r.product_id', $where['product_id']);
        if (isset($where['mer_id']) && $where['mer_id'] > 0) $model = $model->where('p.mer_id', $where['mer_id'])->where('p.status', 1);
        if (isset($where['title']) && $where['title']) {
            $model = $model->join('User u', 'u.uid=r.uid');
            $model = $model->where('r.uid|u.nickname', 'LIKE', "%$where[title]%");
        }
        if (isset($where['store_name']) && $where['store_name']) $model = $model->where('p.store_name', 'LIKE', "%$where[store_name]%");
    ...
    {
        $data = self::setWhere($where)->page((int)$where['page'], (int)$where['limit'])->field('r.*,p.store_name,p.mer_id,p.status')->select();
