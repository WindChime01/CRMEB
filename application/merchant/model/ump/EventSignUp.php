 ...
 * 活动报名
 * Class EventSignUp
 * @package app\merchant\model\ump
 */
class EventSignUp extends ModelBasic
{
    ...
    {
        $data['wz'] = self::statusByWhere('', new self(), '', 0, $mer_id)->count();
        $data['wf'] = self::statusByWhere('', new self(), '', 1, $mer_id)->count();
        $data['yt'] = self::statusByWhere('', new self(), '', -2, $mer_id)->count();
        return $data;
    }
